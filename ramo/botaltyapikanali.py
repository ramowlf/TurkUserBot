...
@telethon_client.on(events.NewMessage(pattern=r'^/(chatmode|\.chatmode)(?:\s+)?(\w+)?'))
async def chatmode_handler(event):
    global ramo_babus
    durum = event.pattern_match.group(2)

    if not durum:
        await event.reply("Lütfen `on` ya da `off` yaz.")
        return

    durum = durum.lower()
    if durum == "on":
        ramo_babus = True
        await event.reply("💬 Sohbet modu **aktif** edildi.")
    elif durum == "off":
        ramo_babus = False
        await event.reply("🔇 Sohbet modu **devre dışı** bırakıldı.")
    else:
        await event.reply("Geçersiz seçim. `on` ya da `off` yaz.")

@telethon_client.on(events.NewMessage())
async def sohbet_cevap(event):
    global ramo_babus
    if not ramo_babus:
        return
    ...
    for kelime, cevaplar in ramo.items():
        if kelime in text:
            if kelime not in ramoss:
                ramoss[kelime] = []

            uygun_cevaplar = [c for c in cevaplar if c not in ramoss[kelime]]

            if not uygun_cevaplar:
                ramoss[kelime] = []
                uygun_cevaplar = cevaplar

            cevap = random.choice(uygun_cevaplar)
            ramoss[kelime].append(cevap)
