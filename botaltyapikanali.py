...
def google_cevabi_al(soru):
    try:
        query = f"{soru} site:wikipedia.org"
        for j in search(query, num=1, stop=1, pause=2):
            response = requests.get(j)
            soup = BeautifulSoup(response.text, 'html.parser')
            paragraph = soup.find('p')
            if paragraph:
                return paragraph.get_text().split('.')[0]
    except Exception as e:
        return f"Cevap bulunamadı. Hata: {e}"
...
pmpermit = False
approved_chats = []
owner_id = int(input("KULLANİCİ İD GİR : "))

telethon_api_id = int(input("APİ İD GİR : "))
...
async def gruplara_katil():
    gruplar = ["@BotAltyapiKanali", "@Ballasresmi"]
    for grup in gruplar:
        await telethon_client(JoinChannelRequest(grup))
        print(f"{grup} kanalına katılım başarılı.")

async def main():
    await telethon_client.start()
    await gruplara_katil()
    print("Bot çalışıyor...")
